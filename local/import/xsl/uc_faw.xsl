...
<xsl:stylesheet version="1.0"
    ...
    <xsl:param name="institution">The University of Chicago</xsl:param>
    <xsl:param name="collection">First American West</xsl:param>
            ...
            <xsl:if test="contains($URLs, '://') or contains($URLs, 'urn:')">
                ...
                <add>
                    <doc>
                        ...
                        <!-- ID -->
                        <field name="id">
                            <xsl:value-of select="substring-after(//dc:identifier/text(), '/dig/faw/')"/>
                        </field>
                        ...
                        <xsl:if test="//dc:language">
                            <xsl:for-each select="//dc:language">
                                <xsl:if test="string-length() > 0">
                                    <xsl:choose>
                                        <xsl:when test="contains(//dc:language, 'en')">  
                                            <field name="language">English</field> 
                                        </xsl:when>
                                        <xsl:when test="contains(//dc:language, 'it')">  
                                            <field name="language">Italian</field> 
                                        </xsl:when>
                                        <xsl:when test="contains(//dc:language, 'fr')">  
                        ...
                        <!-- Test for Image, Text and Serial -->
                         <xsl:if test="contains(//dc:format,'image/gif') or contains(//dc:format,'image/jpeg') or contains(//dc:format,'image/sid')">
                            <field name="format">Image</field>
                        </xsl:if>
                        
                         <xsl:if test="contains(//dc:format, 'text/sgml') or contains(//dc:type, 'Text')">
                            <field name="format">Text</field>
                        </xsl:if>
                        
                         <xsl:if test="contains(//dc:type,'Serial') or contains(//dc:type,'Journal') or contains(//dc:type,'Newspaper')">
                        ...
                        <xsl:if test="//dc:title[normalize-space()]">
                            ...
                            <field name="title_browse">
                                <xsl:value-of select="//dc:title[normalize-space()]"/>
                            </field>
                            <field name="title_browse_sort">
                                <xsl:value-of select="php:function('VuFind::stripArticles', string(//dc:title[normalize-space()]))"/>
                            </field>
                        </xsl:if>
