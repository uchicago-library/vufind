    ...
    public function createAction()
    {
        ...
        if (!is_null($this->params()->fromPost('submit', null))) {
            ...
            if (count($messages) == 0) {
                ...
                $catalog = $this->getILS();
                $profile = $catalog->getMyProfile(array('barcode' => $form_barcode, null));

                if (!array_key_exists('firstname', $profile) || 
                    !array_key_exists('lastname', $profile) ||
                    !array_key_exists('email', $profile)) {
                    ...
                    if (strtolower($lastname) != strtolower($form_lastname)) {
				        ...
				        $sql = "INSERT INTO user VALUES (NULL, :barcode, :pin, NULL, :firstname, :lastname, :email, :barcode, :catpassword, NULL,'','','', :date, '')";
				        $sth = $this->db->prepare($sql);
				        $r = $sth->execute(array(
	                        ':barcode' => $form_barcode,
	                        ':pin' => $form_pin1,
                        ...
                        if ($r == false || $affected_rows != 1) {
                            ...
                            return $this->forwardTo('AdminPin', 'Home');
                        }
                    }
                }
            }
